...
public class StateFileDAO implements StateDAO {
	...
	public void init() {
		...
		try (
			...
			while ((line = buf.readLine()) != null) {
				...
				String flag = tokens[8];
				states.add(new State(num, abbrev, name, capital, latitude, longitude, population, bird, flag));
			}
		} catch (Exception e) {
	...
	@Override
	public State getStateNum(int num)
	{
		State s = null;
		
		for (State state : states) {
			if(state.getStateNum() == num){
				s= state;
			}
		}
		return s;
